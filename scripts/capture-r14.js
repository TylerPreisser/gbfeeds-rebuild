// scripts/capture-r14.js — R14 audit screenshots
// - Desktop 1440×900: 7 routes (home pre-counter + post-counter)
// - Desktop 1280×800: home only (NavBar tagline crowding test)
// - Tablet 768×1024: 4 routes
...
const OUT_DIR = '/Users/tylerpreisser/Downloads/gbfeeds.com-rebuilt/.context/screenshots/rebuild-r14';
...
const HOME_ONLY = [{ path: '/', slug: 'home' }];

const VIEWPORTS = [
  { name: 'desktop_1440', width: 1440, height: 900, routes: FULL_ROUTES },
  { name: 'desktop_1280', width: 1280, height: 800, routes: HOME_ONLY },
  ...
  for (const viewport of VIEWPORTS) {
    ...
    for (const route of viewport.routes) {
      ...
      try {
        ...
        if (route.slug === 'home') {
          // 1) Pre-counter snapshot — taken IMMEDIATELY at networkidle (no scroll)
          ...
          // 2) Post-animation: scroll counter into view, wait 5s for tween + photo cycles
          await page.evaluate(() => {
